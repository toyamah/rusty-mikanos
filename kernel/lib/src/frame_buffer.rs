...
impl FrameBuffer {
    pub fn new(mut config: FrameBufferConfig) -> Self {
        let mut buffer = if config.frame_buffer.is_null() {
    ...
    pub fn copy(&self, pos: Vector2D<usize>, src: &FrameBuffer) {
        assert!(self.config.pixel_format == src.config.pixel_format);

        let dst_width = self.config.horizontal_resolution as usize;
        let dst_height = self.config.vertical_resolution as usize;
        let src_width = src.config.horizontal_resolution as usize;
        let src_height = src.config.vertical_resolution as usize;
        ...
        let i = bytes_per_pixel * (pixels_per_scan_line * copy_start_dst_y + copy_start_dst_x);
        ...
        for _ in 0..copy_end_dst_y - copy_start_dst_y {
            unsafe {
                copy_nonoverlapping(dst_buf, src_buf, bytes_per_copy_line);
